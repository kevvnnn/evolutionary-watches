...
void WatchFileIO::writeWatch(std::ostream& out, const Genome::Watch& watch) const {
    ...
    for (const auto& compPtr : watch.getAllComponents()) {
        ...
        double x = comp->getX(); out.write(reinterpret_cast<const char*>(&x), sizeof(double));
        double y = comp->getY(); out.write(reinterpret_cast<const char*>(&y), sizeof(double));
...
std::unique_ptr<Genome::Watch> WatchFileIO::readWatch(std::istream& in) const {
    ...
    for (unsigned int i = 0; i < compCount; ++i) {
        ...
        if (type == 1) { // Gear
            ...
            auto gear = std::make_unique<Gear>(cname, w, f, x, y, tc, dia, mq);
            if (hj) {
                double jh; bool jc;
                in.read(reinterpret_cast<char*>(&jh), sizeof(double));
                in.read(reinterpret_cast<char*>(&jc), sizeof(bool));
                gear->setJewel(std::make_unique<Jewel>("Jewel_For_"+cname, 0.01, 0.005, 0, 0, jh, jc));
            }
            ...
            watch->addComponent(std::make_unique<BalanceWheel>(cname, w, f, x, y, moi, iso, amp));
        } 
            ...
            watch->addComponent(std::make_unique<Spring>(cname, w, f, x, y, static_cast<Spring::SpringType>(stype), el, fr, len));
        } 
        else if (type == 4) { // Hand
            ...
            watch->addComponent(std::make_unique<Hand>(cname, w, f, x, y, static_cast<Hand::HandType>(htype), len, bal));
        } 
            ...
            watch->addComponent(std::make_unique<Jewel>(cname, w, f, x, y, hard, cap));
        }
    }
    return watch;
}
